...
public class ResExceptionHandler extends DefaultErrorWebExceptionHandler {
	...
	protected Map<String, Object> getErrorAttributes(ServerRequest request, boolean includeStackTrace) {
		int status = (int) super.getErrorAttributes(request, includeStackTrace).get("status");
		Throwable error = super.getError(request);
		return response(status, this.buildMessage(request, error));
	}
	...
	private String buildMessage(ServerRequest request, Throwable ex) {
		if(ex instanceof ResponseStatusException && ((ResponseStatusException)ex).getStatus() == HttpStatus.NOT_FOUND) {
			return String.format("%s not found", request.path());
		}
