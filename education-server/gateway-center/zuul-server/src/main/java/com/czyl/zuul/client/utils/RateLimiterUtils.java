package com.czyl.zuul.client.utils;
		...
		if (limit == null) {
			synchronized (RateLimiterUtils.class) {
				...
				if (limit == null) {
					...
					RESOURCE_LIMIT_MAP.put(resource, limit);
					// LoggerUtil.info(RateLimiterUtils.class, "create rate limiter for
					// key:{0},QPS:{1}", resource,qps);
				}
			}
		}
