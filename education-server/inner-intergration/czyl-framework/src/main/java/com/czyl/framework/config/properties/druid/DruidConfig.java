...
import static com.czyl.framework.config.properties.druid.DruidConsts.ASYNC_INIT;
import static com.czyl.framework.config.properties.druid.DruidConsts.CLEAR_FILTERS_ENABLE;
import static com.czyl.framework.config.properties.druid.DruidConsts.FAIL_FAST;
import static com.czyl.framework.config.properties.druid.DruidConsts.FILTERS;
import static com.czyl.framework.config.properties.druid.DruidConsts.INITIAL_SIZE;
import static com.czyl.framework.config.properties.druid.DruidConsts.INIT_CONNECTION_SQLS;
import static com.czyl.framework.config.properties.druid.DruidConsts.INIT_GLOBAL_VARIANTS;
import static com.czyl.framework.config.properties.druid.DruidConsts.INIT_VARIANTS;
import static com.czyl.framework.config.properties.druid.DruidConsts.KEEP_ALIVE;
import static com.czyl.framework.config.properties.druid.DruidConsts.KILL_WHEN_SOCKET_READ_TIMEOUT;
import static com.czyl.framework.config.properties.druid.DruidConsts.MAX_ACTIVE;
import static com.czyl.framework.config.properties.druid.DruidConsts.MAX_EVICTABLE_IDLE_TIME_MILLIS;
import static com.czyl.framework.config.properties.druid.DruidConsts.MAX_POOL_PREPARED_STATEMENT_PER_CONNECTION_SIZE;
import static com.czyl.framework.config.properties.druid.DruidConsts.MAX_WAIT;
import static com.czyl.framework.config.properties.druid.DruidConsts.MAX_WAIT_THREAD_COUNT;
import static com.czyl.framework.config.properties.druid.DruidConsts.MIN_EVICTABLE_IDLE_TIME_MILLIS;
import static com.czyl.framework.config.properties.druid.DruidConsts.MIN_IDLE;
import static com.czyl.framework.config.properties.druid.DruidConsts.NOT_FULL_TIMEOUT_RETRY_COUNT;
import static com.czyl.framework.config.properties.druid.DruidConsts.PHY_TIMEOUT_MILLIS;
import static com.czyl.framework.config.properties.druid.DruidConsts.POOL_PREPARED_STATEMENTS;
import static com.czyl.framework.config.properties.druid.DruidConsts.RESET_STAT_ENABLE;
import static com.czyl.framework.config.properties.druid.DruidConsts.STAT_LOG_SLOW_SQL;
import static com.czyl.framework.config.properties.druid.DruidConsts.STAT_MERGE_SQL;
import static com.czyl.framework.config.properties.druid.DruidConsts.STAT_SLOW_SQL_MILLIS;
import static com.czyl.framework.config.properties.druid.DruidConsts.STAT_SQL_MAX_SIZE;
import static com.czyl.framework.config.properties.druid.DruidConsts.TEST_ON_BORROW;
import static com.czyl.framework.config.properties.druid.DruidConsts.TEST_WHILE_IDLE;
import static com.czyl.framework.config.properties.druid.DruidConsts.TIME_BETWEEN_EVICTION_RUNS_MILLIS;
import static com.czyl.framework.config.properties.druid.DruidConsts.TIME_BETWEEN_LOG_STATS_MILLIS;
import static com.czyl.framework.config.properties.druid.DruidConsts.USE_GLOBAL_DATA_SOURCE_STAT;
import static com.czyl.framework.config.properties.druid.DruidConsts.USE_UNFAIR_LOCK;
import static com.czyl.framework.config.properties.druid.DruidConsts.VALIDATION_QUERY;
...
public class DruidConfig {
	...
	private DruidFilter filter = new DruidFilter();
	...
	public Properties toProperties(DruidConfig g) {
		...
		// stat配置参数
		Integer statSqlMaxSize = this.filter.stat.getSqlMaxSize() == null ? g.filter.stat.getSqlMaxSize() : this.filter.stat.getSqlMaxSize();
		if (statSqlMaxSize != null) {
			properties.setProperty(STAT_SQL_MAX_SIZE, String.valueOf(statSqlMaxSize));
		}

		Boolean logSlowSql = filter.stat.getLogSlowSql() == null ? g.filter.stat.getLogSlowSql() : filter.stat.getLogSlowSql();
		if (logSlowSql != null && logSlowSql) {
			properties.setProperty(STAT_LOG_SLOW_SQL, "true");
		}
		Long slowSqlMillis = filter.stat.getSlowSqlMillis() == null ? g.filter.stat.getSlowSqlMillis() : filter.stat.getSlowSqlMillis();
		if (slowSqlMillis != null) {
			properties.setProperty(STAT_SLOW_SQL_MILLIS, slowSqlMillis.toString());
		}

		Boolean mergeSql = filter.stat.getMergeSql() == null ? g.filter.stat.getMergeSql() : filter.stat.getMergeSql();
